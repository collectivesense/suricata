...
void LogHttpLogRegister (void)
{
    ...
    //#COLLECTIVE_SENSE #HTTP
    if (TRUE == http_log_write_to_file)
    {
        SCLogNotice("Output 'http-log' will write to file. This may cause performance drawbacks.");
    }
    else
    {
        SCLogInfo("Output 'http-log' will not write to file.");
    }
...
//#COLLECTIVE_SENSE #HTTP
static void FillAndSendHTTPInfo(const Packet *p, LogHttpFileCtx *httplog_ctx, htp_tx_t *tx, uint64_t tx_id)
{
    if ( nn_init_http == 0 ) {
        nn_init_http = 1;
...
static TmEcode LogHttpLogIPWrapper(ThreadVars *tv, void *data, const Packet *p, Flow *f, HtpState *htp_state, htp_tx_t *tx, uint64_t tx_id, int ipproto)
{
    ...
    //#COLLECTIVE_SENSE #HTTP
    FillAndSendHTTPInfo(p, hlog, tx, tx_id);

    if (likely(FALSE == http_log_write_to_file))
    {
        goto end;
    }
    //#COLLECTIVE_SENSE_END #HTTP
